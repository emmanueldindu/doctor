...
export default function DoctorChatRoom() {
  ...
  }, [socket, patientId, currentUserId]);
  ...
  const fetchPatientInfo = async () => {
    try {
      setLoading(true);
      // Fetch patient info from appointments
      const response = await api.get('/appointments/doctor/my-appointments');
      const appointments = response.data;
      
      const patientAppointment = appointments.find((apt: any) => apt.patientId === patientId);
      
      if (patientAppointment) {
        setPatient({
          id: patientAppointment.patientId,
          name: patientAppointment.patient?.name || 'Patient',
          gender: patientAppointment.patient?.gender || '',
        });
      } else {
  ...
  const fetchMessages = async () => {
    try {
      const response = await api.get(`/messages/conversation/${patientId}`);
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {patient?.gender ? `${patient.gender.charAt(0) + patient.gender.slice(1).toLowerCase()} Patient` : 'Patient'}
                </p>
              </div>
            </div>
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            {messages.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">Start a conversation with {patient?.name}</p>
              </div>
            ) : (
